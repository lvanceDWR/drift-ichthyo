...
---
YBFMP Ichthyoplankton Data 1998 - 2022
...
## species code for Sacramento Pikeminnow changed - code updated in CSV before reading in
...
6. Clean up columns and formatting for the sampling data from Excel, for ease of combining data before binding
```{r}
...
8. Now clean up the columns and formatting for the lab data that came back 2019 onward that is housed in Excel. This will eventually be combined with the data from Access so that the complete data set is in one data frame.
```{r}
...
9. Add in some updates to the species dataframe to account for changes in contractor notation of ID'd species
```{r}
...
10. Merge each section of data with catch data before binding rows together
```{r}
#2020 overlap between access and excel
...
#export bound file
write_csv(IchFullData, "R_write/IchFullData1998-2022.csv")

```

11. Ensure columns are in a good order, Find unique values. Create data frame with water year and water type added in for analysis in QA/QC.
```{r}
...
#find where multiple types of comments exist for one event_id 
...
```{r}
SamplingQAQC <- IchFullData[duplicated(IchFullData$event_id) == FALSE,c("event_id", "Datetime","PhysicalDataID",
                                                      "Station","ConditionCode","MeterSetTime","FlowMeterSpeed",
                                                      "FlowMeterStart", "FlowMeterEnd", "TowLocation",
                                                      "SampleVolume", "SamplingAltered", "LarvalDataID",
                                                      "FieldComments", "LarvalCatchID","LabComments",
                                                      "comment_extra")] %>% 
  arrange(Datetime)
...
18. Now that file is notated, read back in and select for the flags and comments. 
```{r}
...
25.Apply outlier definitions to data
...
27. Flowmeter Flags and New Flowdiff calculated
...
28. Add comment about what was flagged (FM for flowmeter-related)
29. Calculate replacement values where Flag_FM = 3 or Comment_SAMP = FM
...
39. Join taxa and clean up order of variables
```{r}
...
ich_select <- CPUEQAQC %>%
  ...
  select(c(event_id, Datetime, Station,
           ...
           SetTime, FlowMeterSpeed, FlowMeterStart, FlowMeterEnd, Flowdiff, FlowdiffAdj, Volume, VolumeAdj,LabComments, Category, LifeStage, Count, CPUE, CPUEAdj, FlagPhys, CommentPhys, FlagSamp, CommentSamp, FlagLab, CommentLab, FlagFM, CommentFM, FlagCPUE, CommentCPUE)) 

 

############## CHECK
# Join taxa table with the rest of the data
# ich_all <- left_join(ich_select, tax) %>%
