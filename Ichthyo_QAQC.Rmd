...
IchSample2 <- IchSample %>%
  ...
  select(-c(SamplingEnteredByAccess, SamplingQAQCByAccess,
            ...
            Bottom_StartTime, "FlowMeterSpeed.x", "MeterSetTime", "FlowMeterStart",
            "FlowMeterEnd", "FieldComments.x", "ConditionCode.x")) %>%
...
IchAccess$Date<-as.Date(IchAccess$Date,"%m/%d/%Y")
IchAccess$Year <- year(IchAccess$Date)
IchAccess$Month <- month(IchAccess$Date)
mymonths <- c("Jan","Feb","Mar",
              "Apr","May","Jun",
              "Jul","Aug","Sep",
              "Oct","Nov","Dec")
IchAccess$MonthAbb <- mymonths[IchAccess$Month ]
IchAccess$Datetime = paste(IchAccess$Date, IchAccess$Time)
IchAccess$Datetime <- ymd_hm(IchAccess$Datetime)
IchAccess$Time <- strptime(IchAccess$Time, format = "%H:%M", tz = "") %>%
  strftime(IchAccess$Time, format = "%H:%M:%S", tz = "", usetz = FALSE)
IchAccess$Time <- hms::as_hms(IchAccess$Time)


IchAccessA$Date<-as.Date(IchAccessA$Date,"%m/%d/%Y")
IchAccessA$Year <- year(IchAccessA$Date)
IchAccessA$Month <- month(IchAccessA$Date)
IchAccessA$MonthAbb <- mymonths[IchAccessA$Month ]
IchAccessA$Datetime = paste(IchAccessA$Date, IchAccessA$Time)
IchAccessA$Datetime <- ymd_hm(IchAccessA$Datetime)
IchAccessA$Time <- strptime(IchAccessA$Time, format = "%H:%M", tz = "") %>%
  strftime(IchAccessA$Time, format = "%H:%M:%S", tz = "", usetz = FALSE)
IchAccessA$Time <- hms::as_hms(IchAccessA$Time)
...
IchLabData <- IchLabData %>%
  ...
  select(-c(SamplingNumber, SampleID, Program)) %>%
  mutate(Date=mdy(Date),DateTime = ymd_hm(paste(as.character(Date), Time)), Time= NULL)
...
6. Update the species data frame to match updates to how the contractors ID species. 
```{r}
